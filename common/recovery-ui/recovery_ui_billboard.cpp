...
static int select_locale(const char* loc, const char* locale) {
 ...
 *   image_0.png
 */
static char *find_prefix(ZipArchive *zip,
                            const char *sku,
...
void BillboardRecoveryUI::Init() {
    ...
    result = create_billboard_surface(filename, locale, &showtime, &surface);
...
void BillboardRecoveryUI::SetZipArchive(ZipArchive *zip)
{
    if (zip == NULL) {
        billboard_frames = 0;
        return;
...
int BillboardRecoveryUI::ExtractBillboards(ZipArchive *zip) {
    int i, fd = 0;
    char path[256];
    ...
    for (i = 0; i < MAX_BILLBOARD_IMAGES; i++) {
        ...
        mzExtractZipEntryToFile(zip, entry, fd);
        close(fd);
    }
    LOGI("Extracted %d billboard images\n", i);
    free(prefix);
    ...
    LOGI("Found %d billboard images in resource\n", i);
    return i;
}
...
int BillboardRecoveryUI::LoadBillboards(int frames) {
    ...
    for (int i = 0; i < frames; i++) {
        ...
        result = create_billboard_surface(filename, locale, &showtime, &surfaces[i]);
        if (result < 0)
...
void BillboardRecoveryUI::LoadBitmap(const char* filename, GRSurface **surface) {
    ...
    int result = create_billboard_surface(fn, locale, NULL, surface);
    if (result < 0) {
        LOGE("missing bitmap %s\n(Code %d)\n", filename, result);
    }
}
