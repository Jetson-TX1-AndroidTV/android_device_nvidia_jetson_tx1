...
static void usage(FILE* where, int argc, char* argv[])
{
    fprintf(where,
            ...
            "  %s dump-slots-info                - Prints info for slots.\n"
            "\n"
...
static int do_dump_slots_info(boot_control_module_t *module)
{
    module->dumpSlotInfo(module);
    return EX_OK;
}
...
int main(int argc, char *argv[])
{
    ...
    } else if (strcmp(argv[1], "dump-slots-info") == 0) {
        return do_dump_slots_info(module);
    } else {
        usage(stderr, argc, argv);
        return EX_USAGE;
    }
